...
const AUTH_BASE = `http://127.0.0.1:${process.env.PORT || 4000}`;
...
const testData = {
	...
	inventoryReport: {
		...
		recommendations: ["Order more of Item 2", "Immediately restock Item 3"],
...
async function getAuthToken() {
	try {
		...
		const authResponse = await axios.post(`${AUTH_BASE}/api/auth/login`, {
...
async function testDirectApi() {
	...
	try {
		...
		const response = await axios.post(
			`${BASE_URL}/low-stock-alert`,
			testData.lowStockAlert,
			{
...
async function testUtilityFunctions() {
	...
	try {
		...
		console.error(
			"❌ Inventory report email utility exception:",
			error.message
		);
	}
}
