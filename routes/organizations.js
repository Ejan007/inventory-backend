 ...
 * Supports: /api/organization/users, /api/organizations/users, /organization/users
 */
router.get("/users", authenticateToken, async (req, res) => {
	try {
		...
		console.error("Error listing organization users:", e);
		return res
			.status(500)
...
const setupHandler = async (req, res) => {
	try {
		...
		if (!organizationId) {
			return res
				.status(400)
				.json({ error: "User is not associated with an organization" });
		}
		...
		const industryValue =
			orgSettings && orgSettings.industry
				? orgSettings.industry.toUpperCase()
				: "OTHER";
		...
		if (orgSettings.contactEmail !== undefined)
			orgUpdateData.contactEmail = orgSettings.contactEmail;
		if (orgSettings.contactPhone !== undefined)
			orgUpdateData.contactPhone = orgSettings.contactPhone;
		if (orgSettings.logoUrl !== undefined)
			orgUpdateData.logoUrl = orgSettings.logoUrl;
		if (defaultCategories !== undefined)
			orgUpdateData.defaultCategories = defaultCategories;
		...
		console.log(
			...
			Array.isArray(req.body.stores)
				? req.body.stores.length + " stores"
				: "Single store"
		);

		const storesData =
			Array.isArray(req.body.stores) && req.body.stores.length > 0
				? req.body.stores
				: [
						{
		...
		console.log(
			"Store ID mappings:",
			JSON.stringify(Array.from(storeIdMap.entries()))
		);

		// Create items with correct store associations
		if (Array.isArray(items) && items.length > 0) {
			console.log(
				`Creating ${items.length} items across ${createdStores.length} stores`
			);
			...
			for (const item of items) {
				...
				if (item.tempStoreId && storeIdMap.has(item.tempStoreId)) {
					...
					storeIdMap.has(item.storeIndex)
				) {
				...
				console.log(
					`Creating item "${item.name}" for store ID: ${targetStoreId}`
				);
